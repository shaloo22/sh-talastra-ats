...
function PocHeader({ setData }) {
  ...
  const filterShowClosedClients = async () => {
    try {
      const dataToSend = selectedClient
        ? { status: "INACTIVE", client_id: selectedClient }
        : { status: "INACTIVE" };
      ...
      console.error("Error fetching closed POCs:", error);
      setData([]);
    }
  };
  ...
  const filterShowActiveJobs = async () => {
    try {
      const dataToSend = selectedClient
        ? { status: "ACTIVE", client_id: selectedClient }
        : { status: "ACTIVE" };
      const response = await axios.post(
        "http://localhost:8080/client/poc/list",
        dataToSend
      ...
      console.error("Error fetching active POCs:", error);
      setData([]);
    }
  };
